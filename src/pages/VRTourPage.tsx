...
const VRTourPage: React.FC<VRTourPageProps> = ({ onBackToHome }) => {
  ...
  useEffect(() => {
    const loadPano2VR = () => {
      ...
      loadScript("/pano2vr/3rdparty/lottie/lottie.min.js")
        ...
        .then(() => {
          ...
          if (window.console && window.console.error) {
            ...
            window.console.error = function (...args) {
              ...
              if (
                message.includes("_lottie_10.onclick") ||
        ...
        .catch((error) => {
          console.error(
            "Erreur lors du chargement des scripts Pano2VR:",
            error
          );
          setIsLoading(false);
        });
    ...
    const waitForContainer = () => {
      ...
      const checkContainer = () => {
        ...
        if (
          ...
          console.log(
            `Container non trouvé, tentative ${attempts}/${maxAttempts}...`
          );
          setTimeout(checkContainer, 100);
        } else {
          console.error(
            "Timeout: impossible de trouver le container après",
            maxAttempts,
            "tentatives"
          );
          setIsLoading(false);
    ...
    const initializePano2VR = () => {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "Impossible d'ajouter le listener nodechange:",
            listenerError
          );
        }
        ...
        const loadConfig = () => {
          if (typeof pano.readConfigUrlAsync === "function") {
            ...
            const result = pano.readConfigUrlAsync(
              "/pano2vr/pano.xml?ts=90345204"
            );
            if (result && typeof result.then === "function") {
              result
                ...
                .catch((error: any) => {
                  console.error(
                    "Erreur lors du chargement de la configuration (async):",
                    error
                  );
                  setIsLoading(false);
                });
            ...
            console.log(
              "Méthodes disponibles sur pano:",
              Object.getOwnPropertyNames(pano)
            );
            setIsLoading(false);
          }
        };
  ...
  const handleSceneChange = (sceneIndex: number) => {
    if (!isInitialized || sceneIndex === activeScene || !panoRef.current) {
      console.log("Changement de scène ignoré:", {
        isInitialized,
        sceneIndex,
        activeScene,
        hasPano: !!panoRef.current,
      });
      return;
    }
