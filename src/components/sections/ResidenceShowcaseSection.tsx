...
const ResidenceShowcaseSection: React.FC = () => {
  ...
  const sectionData = {
    fr: {
      ...
      description: "Une résidence d'exception où l'architecture contemporaine s'harmonise parfaitement avec la nature luxuriante de Nosy Be.",
      features: [
    ...
    en: {
      ...
      unit: "SQM", 
      subtitle: "with Green spaces",
      description: "An exceptional residence where contemporary architecture harmoniously blends with the lush nature of Nosy Be.",
      features: [
    ...
    <section className="relative min-h-screen overflow-hidden">
      ...
      >
        <div className={`absolute inset-0 transition-all duration-2000 ${
          ...
          <div className={`absolute inset-0 bg-gradient-to-br from-black/60 via-transparent to-black/40 transition-opacity duration-1500 ${
            backgroundVisible ? 'opacity-100' : 'opacity-80'
          }`} />
          <div className={`absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent transition-opacity duration-2000 delay-300 ${
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div 
              ...
              className={`space-y-8 transition-all duration-1500 ${
                contentVisible ? 'showcase-slide-in-left visible' : 'showcase-slide-in-left'
              }`}
            >
              {/* Title avec effet typographique */}
              <div className="space-y-4">
                <h2 className="text-4xl lg:text-6xl font-bold text-white leading-tight">
                  <span className="block">{sectionData[language].title.split(' ').slice(0, -1).join(' ')}</span>
              ...
              <div className="grid grid-cols-2 gap-4">
                {sectionData[language].features.map((feature, index) => (
                  <div 
                    key={index}
                    className={`flex items-center space-x-3 transition-all duration-800 delay-${index * 100} ${
                      contentVisible ? 'showcase-fade-in-up visible' : 'showcase-fade-in-up'
                    }`}
                  >
              ...
              <div className={`pt-6 transition-all duration-1000 delay-600 ${
                contentVisible ? 'showcase-slide-in-up visible' : 'showcase-slide-in-up'
              }`}>
                <button className="group relative px-8 py-4 bg-gradient-to-r from-blue-500 to-teal-400 text-white font-semibold rounded-2xl shadow-2xl hover:shadow-blue-500/25 transition-all duration-500 hover:scale-105 overflow-hidden">
                  <span className="relative z-10">
                    {language === 'fr' ? 'Découvrir la Résidence' : 'Discover the Residence'}
                  </span>
            ...
            <div className="relative flex justify-center lg:justify-end">
              <div 
                ...
                className={`relative transition-all duration-2000 delay-800 ${
                  logoVisible ? 'showcase-float-in visible' : 'showcase-float-in'
                }`}
              >
