...
const VRTourButton: React.FC<VRTourButtonProps> = ({ onOpenVRTour }) => {
  ...
  return (
    ...
    >
      {/* Main Button */}
      <div
        className={`relative flex items-center transition-all duration-300 rounded-xl space-x-2 sm:space-x-3 px-3 py-2 sm:px-4 sm:py-3 lg:px-6 lg:py-3 ${
          isHovered
      ...
      >
        ...
        {/* Text Content - Hidden on very small screens, visible on sm+ */}
        ...
        {/* Mobile-only text - Just "VR" on very small screens */}
        ...
        <div
          className={`hidden sm:block transform transition-all duration-300 ${
      ...
      {/* Tooltip on hover - Only show on larger screens */}
      <div
        className={`hidden lg:block absolute top-full mt-2 left-1/2 transform -translate-x-1/2 transition-all duration-300 pointer-events-none z-50 ${
      ...
      <div
        className={`absolute inset-0 rounded-xl bg-gradient-to-r from-blue-600/20 to-purple-600/20 blur-xl transition-opacity duration-300 -z-10 ${
